...
char c[10];
int ok=0,op=1;
double startTime,runTime,curTime;
//方向定义
enum Direction {
	LEFT  = 0,
	UP    = 1,
...
void drawWinTip()
{
	...
	setcolor(BLACK);
	setfont(30, 0, "宋体");
	xyprintf(410, 320, text[0]);
	xyprintf(300, 350, "消耗时间：%.3lfs", curTime);
	xyprintf(305, 380, "已成功击败99%的人");
}
...
int main()
{
	...
	setcaption("The labyrinth game");
	setbkmode(TRANSPARENT);
	music.OpenFile("C:/Users/10339/Desktop/shengyin.mp3");
	if(music.IsOpen())
	{
		cout<<1<<endl;
	}
